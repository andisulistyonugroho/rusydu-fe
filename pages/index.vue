...
getMyAccounts();
callHook("setHeader", "Akun Harta");

const alert = ref(true);
</script>
<template>
  <v-app-bar extended class="border-b">
    <v-app-bar-nav-icon @click="callHook('openDrawer')"></v-app-bar-nav-icon>
  ...
  <div>
    <v-list>
      <v-list-item
        ...
        :title="row.title"
        :to="`/accounts/history?accountId=${row.id}`"
      >
    ...
    <div class="text-center pt-5">
      <v-btn variant="tonal" to="/accounts/form" class="text-capitalize"
        >tambah akun</v-btn
      >
    </div>
  </div>
