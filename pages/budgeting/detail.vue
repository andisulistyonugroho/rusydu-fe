...
<template>
  ...
  <v-list lines="one">
    <v-list-item v-for="row in budgets" :title="row.title">
      <div
        :class="`text-caption ${row.amountLeft === 0 ? `text-green-darken-1` : ``}`"
      >
      ...
      <template v-slot:append>
        <v-menu>
          ...
          <v-list density="compact">
            <v-list-item
              v-if="row.amountLeft > 0"
              @click="openDialog(row.id, row.title, row.amount)"
            >
              <v-list-item-title>Bayar</v-list-item-title>
            </v-list-item>
            <v-list-item
              v-if="row.amountLeft > 0"
              @click="setAsComplete(row.id)"
            >
            ...
            <v-list-item
              v-if="row.amountLeft > 0"
              @click="openBudgetDebt(row.id, row.title, row.amountLeft)"
            >
  ...
  <LazyInputBudgetRealisation
    :budgetid="selectedData.budgetId"
    :budgettitle="selectedData.selectedTitle"
    :dialog="dialog"
    :amount="selectedData.selectedAmount"
    :period="thePeriod"
    @closeit="closeIt"
  />
  <LazyBudgetDebt
    :dialog="debtDialog.dialog"
    :budgetid="debtDialog.budgetId"
    :title="debtDialog.title"
    :amountleft="debtDialog.amountLeft"
    @closeit="closeIt"
    @reloadparent="budgetDebtReloadParent"
  />
</template>
