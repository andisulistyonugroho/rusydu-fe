...
$bus.$emit('set-header', 'Salin Budget')
const months = [
...
const doSubmit = $debounce(async () => {
  try {
    ...
    const data = budgets.value.filter(obj => selectedBudget.value.includes(obj.id))
...
const getBudget = $debounce(async () => {
  try {
    $bus.$emit('wait-dialog', true)
      ...
      <v-col cols="12">
        <v-form ref="form" lazy-validation>
          <v-row no-gutters>
            ...
            <v-col cols="6">
              <v-select v-model="yearfrom" :items="years" label="Tahun" variant="underlined" />
            </v-col>
            <v-col cols="6">
              <v-select v-model="monthfrom" label="Bulan" :items="months" variant="underlined" />
            </v-col>
            <v-col cols="12">
              <v-btn variant="tonal" @click="getBudget()" block>lihat</v-btn>
            </v-col>
            <v-col cols="12">
              <v-checkbox-btn v-if="budgets.length" v-model="selectAll" label="Pilih Semua" class="pt-5" />
              <v-list>
                <v-list-item v-for="row in budgets" :title="row.title">
                  <template v-slot:prepend="isActive">
                    <v-list-item-action start>
                      <v-checkbox-btn :value="row.id" v-model="selectedBudget"></v-checkbox-btn>
                    </v-list-item-action>
                  </template>
            ...
            <v-col v-if="selectedBudget.length" cols="12" class="text-tiny">Tujuan</v-col>
